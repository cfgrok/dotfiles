...
" Coc configuration
if !empty(glob('~/.config/vim/plugged/coc.nvim')) && &rtp =~ 'coc.nvim'
...
if !empty(glob('~/.config/vim/plugged/NeoSolarized')) && &rtp =~ 'NeoSolarized'
...
if !empty(glob('~/.config/vim/plugged/webapi-vim')) && &rtp =~ 'webapi-vim' && !empty(glob('~/.config/vim/vim_snippets_custom.json'))
